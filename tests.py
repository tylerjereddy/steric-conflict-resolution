...
class TestSimpleCopiesDPPC(unittest.TestCase):
    ...
    def test_resolution_steric_conflicts(self):
        subprocess.call('docker run -v {current_path}/test_data/dppc_simple_copies/:/steric_conflict_resolution_work tylerreddy/steric-conflict-resolution /bin/bash -c "python /steric_conflict_resolution/run_steric_resolution.py -input_coord_file_path /steric_conflict_resolution_work/dppc_simple_copies.gro -index_list 1 24 -residue_names_list DPPC -cutoff 2.0 -list_particles_per_residue 12 -output_path /steric_conflict_resolution_work -topology_filepath /steric_conflict_resolution_work/sys.top"'.format(current_path=os.getcwd()), shell=True)
